...
public class LibraryRepository {
    ...
    // Master structures: fast ID lookup + sortable list
    ...
    public HashMap<String, Song> byId() {
        return byId;
    }
            ...
            Files.walk(folder)
                    ...
                    .forEach(p -> {
                        try {
                            ...
                            String title = stripExtension(p.getFileName().toString());
                            ...
                            System.err.println(
                                    "Failed to import file: " + p + " — " + e.getMessage()
                            );
                        }
                    });

        } catch (IOException e) {
            throw new IOException("Error while scanning folder: " + folder, e);
        }
    }
